...
void KeyExpansion(string KeySchedule[4][44], string Key[4][4])
{
	for (int i = 0; i < 4; ++i)
		for (int j = 0; j < 4; ++j)
...
void Padding(string &msg)
{
	while(msg.size() % 16 != 0)
		msg += '0';
}

string BlockEncrypt(string KeySchedule[4][44], string msg)
{
	string State[4][4];
	SState(State, msg);
	Arr_to_hex(State);
...
string BlockDecrypt(string KeySchedule[4][44], string msg)
{
	string State[4][4];
	InvSState(State, msg);
	AddRoundKey(State, KeySchedule, 40);
...
string Encrypt(string msg, string KeySchedule[4][44])
{
	if(msg == "")
		return "";
	Padding(msg);
	...
	for(int i = 0; i < msg.size(); i+=16)
		{
			...
			ans += BlockEncrypt(KeySchedule, msg1);
			msg1 = "";
		}
	return ans;
}

string Decrypt(string msg, string KeySchedule[4][44])
{
	if(msg == "")
		return "";
	string ans = "";
	string msg1 = "";
	for(int i = 0; i < msg.size(); i += 32)
	{
		for(int j = i; j < 32 + i; ++j)
		{
			msg1 += msg[j];
		}
		ans += BlockDecrypt(KeySchedule, msg1);
		msg1 = "";
	}
	string res = "";
	for (int i = 0; i < ans.size(); i += 2)
