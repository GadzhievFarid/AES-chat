...
int __cdecl main(int argc, char **argv) 
{
	...
	iResult = send( ConnectSocket, Name, (int)strlen(Name) + 1, 0 );
	if (iResult == SOCKET_ERROR) {
	...
	do {
		...
		if ( iResult > 0 )
		{
			...
			cout << "You connected to " << recvbuf << endl;
			strcpy_s(ServerName, recvbuf);
		}
		else if ( iResult == 0 )
	...
	while (true)
	{
		...
		if(msg == "Exit")
		{
			...
			iResult = send(ConnectSocket, msg.c_str(), msg.size() + 1, 0 );
			closesocket(ConnectSocket);
			WSACleanup();
			return 1;
		}
